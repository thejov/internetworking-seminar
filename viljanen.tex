...
\subsubsubsection{How the classic Ant Colony Optimization algorithm works}
...
% explain basic AOC principles (Saleem et al) and state how A-ESR differs from those.
%-tehdään ns path sämpläystä, eli kokeillaan eri polkuja eri pisteiden välillä.
%	-sämpläys toteutetaan forward ja backward anteilla (agenteilla)
%		-forward ant liikkuu feromonijälkien "suositusten" ja lokaalin heuristiikkafunktion yhdistelmää (stochastic rule) käyttäen kohti seuraavaa nodea pisteestä pisteeseen b. paremmaksi merkittyjä polkuja suositaan siis paremmalla todennäköisyydellä.
%		-fw ant kerää samalla tietoa reitistä.
%		-saavuttuaan maaliin ns backward ant menee samaa reittiä takaisin ja päivittää joka noden feromonitauluja.
%-feromonitaulussa on tieto siitä, miten hyvä reitti on mennä naapurinoden kautta määränpäähän.
%mitkä ovat A-ESR:n erot normi AOC:iin verrattuna?

\subsubsubsection{How the A-ESR algorithm differs from classic AOC}
...
% http://tex.stackexchange.com/questions/32694/table-exceeds-page
